...
def bi_K_means(dataset, k, dist_meas=dist_eclud):
    ...
    while (len(cent_list) < k):
        ...
        best_clust_ass[np.nonzero(best_clust_ass[:, 0].A==0)[0], 0] = best_cent_to_split # 再次聚类后的第一个簇
        best_clust_ass[np.nonzero(best_clust_ass[:, 0].A==1)[0], 0] = len(cent_list) # 再次聚类后的第二个簇的中欧给你
