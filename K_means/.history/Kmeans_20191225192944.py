...
def bi_K_means(dataset, k, dist_meas=dist_eclud):
    ...
    while (len(cent_list) < k):
        ...
        for i in range(len(cent_list)):
            ...
            centroid_mat, split_clut_ass = K_means(dataset, 2, dist_meas)
            sse_split = np.sum(split_clut_ass[:, 1]) # 统计所有数据距离中心点的距离
            sse_not_split = np.sum(cluster_assment[np.nonzero(cluster_assment[:, 0].A!=i)[0], 1]) # 统计不分割时的
