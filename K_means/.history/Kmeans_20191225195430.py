...
def bi_K_means(dataset, k, dist_meas=dist_eclud):
    ...
    while (len(cent_list) < k):
        ...
        for i in range(len(cent_list)):
            pst_in_curr_cluster = dataset[np.nonzero(cluster_assment[:, 0].A==i)[0], :]
            centroid_mat, split_clust_ass = K_means(dataset, 2, dist_meas)
            ...
            if (sse_split + sse_not_split) < lowest_sse:
                ...
                best_clust_sse = split_clust_ass.copy()
                lowest_sse = sse_split + sse_not_split
            best_clust_ass[np.nonzero(best_clust_sse[:, 0].A == 1)[0], 0] = len(cent_list)
            best_clust_ass[np.nonzero(best_clust_ass)]



def plot_data_set(filename):
    # 导入数据
    datMat = np.mat(load_data_set(filename))
    # 进行k-means算法其中k为4
